...
class HalideSupervisor(Supervisor):
    ...
    def batch_eval(self, eval_data, output_trace_flag, output_trace_option, process_idx):
        ...
        for batch_idx in range(0, data_size, self.batch_size):
            ...
            print(
                'process start idx: %d batch idx: %d pred expr reward: %.4f pred tree reward: %.4f gt expr reward: '
                '%.4f gt tree reward: %.4f' % \
                (process_idx, batch_idx, cur_avg_expr_reward, cur_cum_tree_reward * 1.0 / len(batch_data),
                 cur_gt_expr_reward * 1.0 / len(batch_data), cur_gt_tree_reward * 1.0 / len(batch_data)))
    ...
    def eval(self, data, output_trace_flag, output_trace_option, output_trace_file, max_eval_size=None):
        ...
        if self.processes == 1:
            cum_loss, cum_expr_reward, cum_tree_reward, cum_gt_expr_reward, cum_gt_tree_reward, trace_rec = self.batch_eval(
                eval_data, output_trace_flag, output_trace_option, 0)
        else:
            cum_loss = 0
            cum_expr_reward = 0
            cum_tree_reward = 0
            ...
            for st in range(0, data_size, batch_per_process):
                res += [pool.apply_async(self.batch_eval, (
                    eval_data[st: st + batch_per_process], output_trace_flag, output_trace_option, st))]
            for i in range(len(res)):
                cur_cum_loss, cur_cum_expr_reward, cur_cum_tree_reward, cur_cum_gt_expr_reward, cur_cum_gt_tree_reward, cur_trace_rec = \
                    res[i].get()
        ...
        print('average: pred expr reward: %.4f pred tree reward: %.4f gt expr reward: %.4f gt tree reward: %.4f' % (
            avg_expr_reward, avg_tree_reward, gt_expr_reward, gt_tree_reward))
