...
class vrpModel(BaseModel):
    ...
    def __init__(self, args):
        ...
        self.policy_embedding = mlp.MLPModel(self.num_MLP_layers, self.LSTM_hidden_size * 6 + self.embedding_size * 2,
                                             self.MLP_hidden_size, self.attention_size, self.cuda_flag,
                                             self.dropout_rate)
        self.policy = mlp.MLPModel(self.num_MLP_layers, self.LSTM_hidden_size * 4, self.MLP_hidden_size,
                                   self.attention_size, self.cuda_flag, self.dropout_rate)
        self.value_estimator = mlp.MLPModel(self.num_MLP_layers, self.LSTM_hidden_size * 4, self.MLP_hidden_size, 1,
    ...
    def rewrite(self, dm, trace_rec, candidate_rewrite_pos, eval_flag, max_search_pos, reward_thres=None):
        ...
        global sample_exp_reward_tensor
    ...
    def forward(self, batch_data, eval_flag=False):
        ...
        for dm_idx, cur_dm_rec in enumerate(dm_rec):
            ...
            for idx, (ac_logprob, pred_reward, cur_pred_reward_tensor, rewrite_pos, applied_op, new_dis) in enumerate(
                ...
                cur_reward_tensor = data_utils.np_to_tensor(np.array([cur_reward], dtype=np.float32), 'float',
                                                            self.cuda_flag)
